...
export default function ProjectInfo({
  ...
  const HandleChangeProject = (e, i) => {
    ...
    if (name === "projectStatus" && value === "done") {
      delete onChange[i].projectSubDate;
    } else if (name === "projectStatus" && value === "in-progress") {
      delete onChange[i].projectDate;
    }
    onChange[i][name] = value;
    setProjectInfo(onChange);
    setMsg({});
  };
  const HandleDeleteProjectImg = (i) => {
    const onChange = [...projectInfo];
    delete onChange[i].projectImg;
    setProjectInfo(onChange);
  };
  const HandleChangeDates = (e, index, dateIndex) => {
    const updatedProjects = [...projectInfo];
    updatedProjects[index].projectSubDate[dateIndex] = e.target.value;
    setProjectInfo(updatedProjects);
  };
  const HandleDeleteDates = (index, dateIndex) => {
    const updatedProjects = [...projectInfo];
    updatedProjects[index].projectSubDate.splice(dateIndex, 1); // Remove the date
    setProjectInfo(updatedProjects);
  };

  // projects info inputs
  const renderProject = projectInfo.map((project, index) => {
    // inputs fields
    const InputFields = [
      {
        type: "text",
                ...
                >
                  ...
                  >
                    ...
                    <SecondaryBtn
                      ...
                      } w-full bg-white !text-darkGreen `}
                    />
                  </div>
                </div>
              );
              ...
              return (
                <>
                  ...
                  {project.projectStatus === "in-progress" && (
                    ...
                    >
                      <p className="text-primary-color2 px-4 m-0 text-start w-full sm:text-sm md:!font-[18px]">
                      ...
                      >
                        ...
                        <SecondaryBtn
                          ...
                          style="w-full bg-white !text-darkGreen !py-3"
                        />
                      </div>
                    </div>
                  )}
